...
bool bit_test(const cpp_int &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    return mpz_tstbit(a.backend().data(), col);
}

bool bit_test(const std::set<pos_t> &a, pos_t col) {
    return a.find(col) != a.end();
}

bool bit_test(const std::vector<pos_t> &a, pos_t col) {
    return std::find(a.begin(), a.end(), col) != a.end();
}

void bit_set(mpz_t &a_d, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    mpz_setbit(a_d, col);
}

void bit_set(cpp_int &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    mpz_setbit(a.backend().data(), col);
}

void bit_set(std::set<pos_t> &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    a.insert(col);
}

void bit_set(std::vector<pos_t> &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    if (!bit_test(a, col)) {
        a.push_back(col);
    }
}
void bit_unset(cpp_int &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    mpz_clrbit(a.backend().data(), col);
}

void bit_unset(std::set<pos_t> &a, pos_t col) {
    auto index = a.find(col);
    if (index != a.end())
        a.erase(index);
}

void bit_unset(std::vector<pos_t> &a, pos_t col) {
...
void bit_toggle(Container &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    if (bit_test(a, col))
        bit_unset(a, col);
    else
...
template void bit_toggle(cpp_int&, pos_t);
template void bit_toggle(std::vector<pos_t>&, pos_t);
template void bit_toggle(std::set<pos_t>&, pos_t);

pos_t next_bit(const cpp_int &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
...
pos_t next_bit(const std::set<pos_t> &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
...
pos_t next_bit(const std::vector<pos_t> &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
...
void clear_after(mpz_t &a_d, pos_t col) {
    //assert(col < static_cast<pos_t>(-1));
...
void clear_after(cpp_int &a, pos_t col) {
    assert(col < static_cast<pos_t>(-1));
    mpz_t& a_d = a.backend().data();
    clear_after(a_d, col);
}

void clear_after(std::set<pos_t> &a, pos_t col) {
    a.erase(a.lower_bound(col), a.end());
}

void clear_after(std::vector<pos_t> &a, pos_t col) {
    auto it = a.begin();
    while (it != a.end()) {
        if (*it >= col) {
            a.erase(it);
